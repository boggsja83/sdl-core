...
/******************************************************************************/
	...
	switch(pec.cm){
		...
		if(ents[pec.owner_id]&CM_TEXTURE){
		    texture[pec.owner_id].rend_i = static_cast<cTexture*>(&pec)->rend_i;
		    texture[pec.owner_id].text_i = static_cast<cTexture*>(&pec)->text_i;
		    texture[pec.owner_id].src = static_cast<cTexture*>(&pec)->src;
		}
		else return ECS_LACKS_COMP;
		break;
	    default:
